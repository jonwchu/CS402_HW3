...
{
    dbg_print("pframe_get: Creating Pframe\n");
    /* Check to see if page is already resident */
    *result = pframe_get_resident(o,pagenum);
    if(*result != NULL)
    {
        /* page is already a resident wait for it not to be */
        while(pframe_is_busy(*result))
        {
            sched_sleep_on(&(*result)->pf_waitq);
        }
        dbg_print("pframe_get: Page is already a Resident\n");
        return 0;
    }
    else
    {
        ...
        {
            /* Low on memory call pageoutd_wakeup */
            pageoutd_wakeup();
        ...
        *result = pframe_alloc(o,pagenum);
        ...
        dbg_print("pframe_get: Result of Fill: %i\n", res);
        /* KASSERT(res == 0); */
...
{
    dbg_print("pframe_pin: Pinning Pframe\n");
    pframe_set_busy(pf);
    if(!pframe_is_pinned(pf))
    {
        npinned++;
...
{
    dbg_print("pframe_unpin: Unpinning Pframe\n");
    pframe_set_busy(pf);
    pf->pf_pincount--;
    if(pf->pf_pincount == 0)
    {
